...
void* threadFunction12(void *var)
{
...
void* threadFunction34(void *var)
{
	int thread_id = *(int*)var;
...
void* threadFunction56(void *var)
{
	int thread_id = *(int*)var;
...
int main(int argc, char *argv[])
{
	...
	for(i = 0; i < P; i++) {
		if (i % 4 == 0) {
			pthread_create(&(tid[i]), NULL, threadFunction12, &(thread_id[i]));
		} else if (i % 4 == 1) {
			pthread_create(&(tid[i]), NULL, threadFunction34, &(thread_id[i]));
		} else if (i % 4 == 2) {
			pthread_create(&(tid[i]), NULL, threadFunction56, &(thread_id[i]));
		} else {
			pthread_create(&(tid[i]), NULL, threadFunction7, &(thread_id[i]));
		} 
	}
